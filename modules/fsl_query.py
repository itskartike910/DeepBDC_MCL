## FSL Query module for feature extraction
import torch
...
from .bdc_module import BDC  # Import BDC from deepBDC
...
class FSLQuery(nn.Module):
    def __init__(self, cfg):
        ...
        # Initialize BDC (deepBDC feature extractor)
        input_dim = (
            self.encoder.out_channels,
            getattr(cfg.model, 'image_height', 84),
            getattr(cfg.model, 'image_width', 84),
        )
        self.bdc = BDC(is_vec=True, input_dim=input_dim, dimension_reduction=getattr(cfg.model,'dimension_reduction', 512), activate='relu')
    ...
    def forward_Grid(self, support_x, support_y, query_x, query_y):
        ...
        # Apply BDC (deepBDC feature extraction)
        support_xf = self.bdc(self.encoder(support_x.view(-1, 3, h, w)))
        support_xf = support_xf.view(b, s, grids_s, -1).permute(0, 1, 3, 2).unsqueeze(-1)
        query_xf = self.bdc(self.encoder(query_x.view(-1, 3, h, w)))
    ...
    def forward_PyramidFCN(self, support_x, support_y, query_x, query_y):
        ...
        # Apply BDC (deepBDC feature extraction)
        support_xf = self.bdc(self.encoder(support_x.view(-1, c, h, w)))
        query_xf = self.bdc(self.encoder(query_x.view(-1, c, h, w)))
